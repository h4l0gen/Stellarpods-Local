...
variable "challenge_dir" {
  type    = string
  default = "secret-of-kurama"  // here i did change
}
...
resource "null_resource" "docker_build" {
  provisioner "local-exec" {
    command = "docker build -t ctf-${var.challenge_name}:latest ${var.download_path}/${var.challenge_dir}"
  }

  depends_on = [null_resource.debug_info]
}
# newly added for differentiate images
variable "challenge_name" {
  type    = string
  default = "secret-of-kurama"  # This should be set dynamically based on the current challenge
}

resource "docker_image" "challenge_image" {
  name = "ctf-${var.challenge_name}:latest"
  keep_locally = false // changed 
...
resource "null_resource" "load_image_to_minikube" {
  provisioner "local-exec" {
    command = "minikube image load ctf-${var.challenge_name}:latest"
  }
